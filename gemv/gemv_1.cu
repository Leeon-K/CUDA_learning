...
//  initialize<T>(vec, d_vec, mat, d_mat, dst, d_dst, M, N);
    ...
    constexpr int THREAD_NUMS = 256; // 每个block 256个线程（max=1024）
    constexpr int VEC_SIZE = 1; // fp32向量长度 vec_size = 4 -->fp4 ;= 1
    constexpr int VECS_PER_THREAD = (N / THREAD_NUMS) / VEC_SIZE; // N / 256 / VEC_SIZE = 2  16不行
    // dispatch kernel 
    DispatchLauncher<VECS_PER_THREAD, VEC_SIZE, THREAD_NUMS>::template launcher<T>(d_mat, d_vec, d_dst, M, N);
    // 为什么这里 VEC_SIZE=1 会出错？ 
