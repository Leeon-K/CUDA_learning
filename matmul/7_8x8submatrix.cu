...
__global__ void gemm_submat8x8(const int m,const int n,const int k,const float alpha, const float *A, const float *B, const float beta, float* C)
{
    // 分配共享内存  
    ...
    int row_b = (tx & 1) << 2, col_b = tx >> 1; // row_b 是thread连续的
    ...
    for (int i = 0; i < k; i += BK){  // 循环次数增多 输入足够大
        // 存储是 列主序, vec load
...
// ------------------------------------------------------------------------------------
        gemm_submat8x8<<<Grid,Block>>>(m,n,k,*alpha,devPtrA,devPtrB,*beta,devPtrC);
