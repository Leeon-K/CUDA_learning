...
__global__ void gemm_shared_plus_reducebc(const int m,const int n,const int k,const float alpha, const float *A, const float *B, const float beta, float* C)
{
    // 分配共享内存  
    ...
    for (int i = 0; i < k; i += BK){
        // sa[IDX2R(row,col,BM)] = A[IDX2R(row,col,m)];
        // sb[IDX2R(col,row,BK)] = B[IDX2R(row,col,k)]; // 一次读取一个方块
        // 存储是 列主序
        sa[IDX2C(row,col,BM)] = A[IDX2C(row,col,m)];
        sb[IDX2C(col,row,BK)] = B[IDX2C(row,col,k)]; // 一次读取一个方块
        A += m<<5; // 一行 一次32行
        B += 32; //小方块一列32
        __syncthreads();
        for (int b_k = 0; b_k < BK; b_k++){
            sum += sa[IDX2C(row,b_k,BM)] * sb[IDX2C(col,b_k,BK)]; // 因为row是连续的
        }
        __syncthreads();
    }
...
// ------------------------------------------------------------------------------------
        gemm_shared_plus_reducebc<<<Grid,Block>>>(m,n,k,*alpha,devPtrA,devPtrB,*beta,devPtrC);
